            ...
            for agent_config in workflow_pipeline:
                ...
                try:
                    if role == "intake":
                        ...
                        # Enrichment: let intake override system_type if it classified a clearer category
                        try:
                        ...
                        # Dynamically extend matched_specialists from triage recommendation
                        try:
                    ...
                    if not agent_config.is_optional:
                        # Non-optional step failed — try to continue with what we have
                        pass
...
async def _run_knowledge_step(alert: Alert, agent_config: FoundryAgentConfig) -> list[dict]:
    """Search knowledge bases (SharePoint + AI Search + RAG pgvector) for relevant docs."""
    from app.services.azure_graph_service import search_sharepoint, search_ai_index

    query = f"{alert.alertname} {alert.alert_category or ''} {alert.summary or ''}"
    docs = []

    # RAG Knowledge Base (pgvector) — opt-in
    try:
        from app.database import async_session
        ...
        import logging
        logging.getLogger(__name__).debug("RAG search skipped in foundry knowledge step: %s", e)

    # AI Search (preferred)
    ai_docs = await search_ai_index(query, max_results=3)
    docs.extend(ai_docs)

    # SharePoint fallback/supplement

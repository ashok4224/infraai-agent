...
class UploadConnector(KnowledgeConnector):
    ...
    async def fetch_documents(self, connection_config: dict, filter_config: dict) -> List[RawDocument]:
        ...
        if app_settings.AZURE_BLOB_ACCOUNT_URL and app_settings.AZURE_BLOB_CONTAINER:
            return await self._fetch_from_blob(
                app_settings, allowed_ext, max_size_kb
            )
        ...
        logger.info("Upload (local): fetched %d documents", len(documents))
        return documents

    async def _fetch_from_blob(self, app_settings, allowed_ext: set, max_size_kb: int) -> List[RawDocument]:
        """Fetch all files from Azure Blob Storage container."""
        try:
                ...
                async for blob in container_client.list_blobs():
                    ...
                    try:
                        ...
                        documents.append(RawDocument(
                            ...
                            file_path=blob.name,
                            doc_type="markdown" if ext == ".md" else "text",
                            metadata={"upload": True, "blob": True},
                        ))
                    except Exception as e:
                        logger.warning("Failed to read blob %s: %s", blob.name, e)

            logger.info("Upload (Azure Blob): fetched %d documents from container %s",
                        len(documents), app_settings.AZURE_BLOB_CONTAINER)
            return documents

        except Exception as e:
            logger.error("Azure Blob fetch failed: %s", e)
            return []
    ...
    async def test_connection(self, connection_config: dict, filter_config: dict) -> dict:
        ...
        if app_settings.AZURE_BLOB_ACCOUNT_URL and app_settings.AZURE_BLOB_CONTAINER:
            try:
                ...
                return {"status": "ok", "storage": "azure_blob",
                        "container": app_settings.AZURE_BLOB_CONTAINER}
            except Exception as e:
                return {"status": "error", "message": str(e)}
        upload_dir = Path(self.UPLOAD_DIR)
        upload_dir.mkdir(parents=True, exist_ok=True)
        return {"status": "ok", "storage": "local", "path": str(upload_dir)}

...
# Two levels up from routers/ ??? app/ ??? uploads/knowledge  (matches UploadConnector path)
...
async def upload_file(
    ...
    # ?????? Azure AI Foundry Files API (preferred ??? fully managed, no extra services) ??????
    if app_settings.AZURE_AI_FOUNDRY_ENDPOINT:
        try:
            from app.services.azure_foundry_service import (
                upload_file_to_foundry,
                add_file_to_knowledge_store,
            )
            file_id = await upload_file_to_foundry(safe_name, content)
            await add_file_to_knowledge_store(file_id)
            logger.info("File '%s' uploaded to Foundry and added to knowledge vector store", safe_name)
            return {
                "filename": safe_name,
                "size_bytes": len(content),
                "storage": "azure_foundry",
                "file_id": file_id,
                "note": "File is being indexed by Azure AI Foundry (auto-chunk + auto-embed)",
            }
        except Exception as e:
            logger.error("Foundry file upload failed, trying Azure Blob fallback: %s", e)

    # ?????? Azure Blob Storage fallback ???????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????
    if app_settings.AZURE_BLOB_ACCOUNT_URL and app_settings.AZURE_BLOB_CONTAINER:
        try:
            from azure.storage.blob.aio import BlobServiceClient
            from azure.identity.aio import ClientSecretCredential, DefaultAzureCredential

            if (app_settings.AZURE_CLIENT_ID and app_settings.AZURE_CLIENT_SECRET
                    and app_settings.AZURE_TENANT_ID):
            ...
            async with BlobServiceClient(app_settings.AZURE_BLOB_ACCOUNT_URL, credential=cred) as blob_service:
                container_client = blob_service.get_container_client(app_settings.AZURE_BLOB_CONTAINER)
                try:
                    await container_client.create_container()
                except Exception:
                    pass
                await container_client.upload_blob(safe_name, content, overwrite=True)

            return {
                "filename": safe_name,
                "size_bytes": len(content),
                "storage": "azure_blob",
                "container": app_settings.AZURE_BLOB_CONTAINER,
            }
        except Exception as e:
            logger.error("Azure Blob upload failed, falling back to local disk: %s", e)
    ...
    return {"filename": safe_name, "size_bytes": len(content), "storage": "local", "path": str(dest)}
